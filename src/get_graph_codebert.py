...
from transformers import RobertaTokenizer, RobertaModel  # 使用 CodeBERT

# 设置参数
WINDOW_SIZE = 9
WORD_EMBEDDINGS_DIM = 768  # CodeBERT 维度
random.seed(1)
torch.manual_seed(1)

# 初始化 CodeBERT 模型
tokenizer = RobertaTokenizer.from_pretrained("microsoft/codebert-base", local_files_only=True)
bert_model = RobertaModel.from_pretrained("microsoft/codebert-base", trust_remote_code=True, use_safetensors=True, local_files_only=True)
bert_model.eval()
bert_model.cuda()  # 如果使用CPU请注释这行

# 嵌入缓存
BERT_EMBEDDING_CACHE = {}

def get_bert_embedding(word):
    if word in BERT_EMBEDDING_CACHE:
        return BERT_EMBEDDING_CACHE[word]
    ...
    BERT_EMBEDDING_CACHE[word] = embedding
    return embedding
