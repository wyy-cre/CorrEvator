...
def main():
    ...
    for i in tqdm(range(10)):
        ...
        train_data, train_labels = process_data(train_group, False)
        test_data, test_labels = process_data(test_group, False)

        data_train_test = []
        data_train_test.extend(train_data)
        data_train_test.extend(test_data)
        label_train_test = []
        label_train_test.extend(train_labels)
        label_train_test.extend(test_labels)
        for _ in range(len(data_train_test)):
            data_train_test[_]["label"] = label_train_test[_]
        new_test_data = []
        for _ in range(int(len(data_train_test) * 0.1)):
            temp = random.choice(data_train_test)
            new_test_data.append(temp)
            data_train_test.remove(temp)
        train_labels, test_labels = [], []
        for _ in data_train_test:
            train_labels.append(_["label"])
        for _ in new_test_data:
            test_labels.append(_["label"])
        train_data = data_train_test
