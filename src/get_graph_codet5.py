...
# 设置参数
WINDOW_SIZE = 9
WORD_EMBEDDINGS_DIM = 768  # CodeT5 的输出维度
random.seed(1)
torch.manual_seed(1)

# 初始化 CodeT5 模型（使用 AutoModel + AutoTokenizer）
tokenizer = AutoTokenizer.from_pretrained("Salesforce/codet5-base")
codet5_model = T5EncoderModel.from_pretrained("Salesforce/codet5-base", trust_remote_code=True, use_safetensors=True)
codet5_model.eval()
codet5_model.cuda()  # 如使用 CPU 请注释

# 嵌入缓存
CODET5_EMBEDDING_CACHE = {}

def get_bert_embedding(word):  # 为保持接口兼容，保留名称
    if word in CODET5_EMBEDDING_CACHE:
        return CODET5_EMBEDDING_CACHE[word]

    inputs = tokenizer(word, return_tensors="pt", truncation=True, max_length=5)
    inputs = {k: v.cuda() for k, v in inputs.items()}  # 如使用 CPU，请删除此行

    with torch.no_grad():
        outputs = codet5_model(**inputs)

    embedding = outputs.last_hidden_state[0].mean(dim=0)  # shape: [hidden_dim]
    CODET5_EMBEDDING_CACHE[word] = embedding
    return embedding
...
def link_graph(g1, g2, label):
    ...
    return Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=torch.tensor(label if label != -1 else 0), node_graph_id=node_graph_id)
